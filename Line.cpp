 ...
 * joeliaco
 *
 * Reads and writes lines for use in pics
 */
...
// Form: (0,0) (4,4) 90 90 90 [start end lineColor]
...
// Constructs a line in form startpoint, endpoint, color values
// EX: (1,4) (4,5) 200 100 45
void Line::read(istream& ins) {
    ...
    // Read and skip specific characters while reading integers
    ins >> nonInt >> xValStart >> nonInt >> yValStart >> nonInt;
    ins >> nonInt >> xValEnd >> nonInt >> yValEnd >> nonInt;
    ins >> redVal >> greenVal >> blueVal;

    // Construct start, end, and color
    start = Point(xValStart, yValStart);
    end = Point(xValEnd, yValEnd);
    lineColor = Color(redVal, greenVal, blueVal);
